...
export function onSave(values) {
   ...
   var data = {
      ...
      title: state.form.NewsForm.values.title,
      content: state.form.NewsForm.values.content,
      titleUrl: state.form.NewsForm.values.titleUrl,
      imgUrl: state.form.NewsForm.values.imgUrl,
      thumbUrl: state.form.NewsForm.values.thumbUrl
   }
...
export function postData(values) {
   const { dispatch, getState, args } = values;
   const state = getState();
   if (validateContent(state.newsReducer.newsForm)) {
      var data = constructData(state, args);
      axios.post('http://localhost:3000/news/save', data).then(function (response) {
...
function constructData(state, args) {
    var data = {
      ...
      tag: args.tag,
      thumbUrl: state.form.NewsForm.values.thumbUrl,
      id:state.newsReducer.newsForm.id?state.newsReducer.newsForm.id:""
...
export function readData(values) {
   ...
   axios.post('http://localhost:3000/news/readdata', config).then(function (response) {
