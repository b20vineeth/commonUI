...
export function postData(values) {
   const { dispatch, getState, args } = values;
   const state = getState();
   if (validateContent(state.reviewReducer.reviewForm)) {
      var data = constructData(state, args);
       axios.post('http://localhost:3000/review/save', data).then(function (response) {
...
function constructData(state, args) {
    var data = {
      ...
      movie: args.movie,
      tag: args.tag,
      reviewPoint: state.form.ReviewForm.values.reviewPoint,
      thumbUrl: state.form.ReviewForm.values.thumbUrl,
      id:state.reviewReducer.reviewForm.id?state.reviewReducer.reviewForm.id:""
...
export function readData(values) {
   ...
   axios.post('http://localhost:3000/review/readdata', config).then(function (response) {
      ...
      if (response.data.successData && response.data.successData.length>0) {
         var data = {
            ...
            reviewPoint: response.data.successData[0].reviewPoint
         }
         dispatch({ type: action.STORE_FORM_DATA, data: data });
